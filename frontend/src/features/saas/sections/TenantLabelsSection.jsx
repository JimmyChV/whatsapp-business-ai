...
const EMPTY_ZONE = { ruleId: '', name: '', color: '#00A884', departments: [], provinces: [], districts: [], departmentId: '', provinceId: '', districtId: '', shippingOptions: [], paymentMethods: { yape: false, plin: false, bank_transfer: false, credit_card: false }, isActive: true };
...
function normZone(item = {}) {
    ...
    return { ruleId: upper(item.ruleId || item.rule_id || ''), name: text(item.name), color: text(item.color || '#00A884') || '#00A884', rulesJson: rules && typeof rules === 'object' && !Array.isArray(rules) ? rules : {}, shippingOptions: normalizeShippingOptions(item.shippingOptions || item.shipping_options), paymentMethods: normalizePaymentMethods(item.paymentMethods || item.payment_methods), isActive: item.isActive !== false && item.is_active !== false };
}
...
function ZonePanel({ busy, requestJson, runAction, setError, canManageLabels, tenantScopeLocked, settingsTenantId }) {
    ...
    const right = <SaasDetailPanel title={detail ? selected.name || 'Zona' : mode === 'create' ? 'Nueva zona' : 'Editar zona'} subtitle={detail ? 'Regla geografica del tenant.' : 'Agrega departamentos, provincias y distritos con selectores jerarquicos.'} className="saas-labels-detail-panel" actions={<>{detail ? <button type="button" disabled={busy || !canManageLabels} onClick={openEdit}>Editar</button> : null}<button type="button" className="saas-btn-cancel" disabled={busy} onClick={close}>Volver</button></>}>
        {detail ? <>
            ...
            <SaasDetailPanelSection title="Metodos de pago"><div className="saas-admin-detail-grid">{Object.entries(paymentMethodLabels).map(([method, label]) => <div key={`payment_read_${method}`} className="saas-admin-detail-field"><span>{label}</span><strong>{paymentMethods[method] ? 'Activo' : 'Inactivo'}</strong></div>)}</div></SaasDetailPanelSection>
        </> : <>
