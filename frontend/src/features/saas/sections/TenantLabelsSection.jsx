...
function ZonePanel({ busy, requestJson, runAction, setError, canManageZones, tenantScopeLocked, settingsTenantId }) {
    ...
    const right = <SaasDetailPanel title={detail ? selected.name || 'Zona' : mode === 'create' ? 'Nueva zona' : 'Editar zona'} subtitle={detail ? 'Regla geografica del tenant.' : 'Agrega departamentos, provincias y distritos con selectores jerarquicos.'} className="saas-labels-detail-panel" actions={<>{detail ? <button type="button" disabled={busy || !canManageZones} onClick={openEdit}>Editar</button> : null}<button type="button" className="saas-btn-cancel" disabled={busy} onClick={close}>Volver</button></>}>
        {detail ? <>
