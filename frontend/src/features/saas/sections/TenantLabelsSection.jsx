...
const EMPTY_ZONE = { ruleId: '', name: '', color: '#00A884', departments: [], provinces: [], districts: [], departmentId: '', provinceId: '', districtId: '', shippingOptions: [], paymentMethods: { yape: false, plin: false, bank_transfer: false, credit_card: false, cash: false }, paymentModality: { ...EMPTY_PAYMENT_MODALITY }, isActive: true };
...
const moneyValue = (v) => (v === null || v === undefined || v === '' ? '' : String(v));
const normalizeMoneyInput = (v) => {
    if (v === null || v === undefined || v === '') return null;
    const n = Number(v);
    return Number.isFinite(n) ? Math.round(n * 100) / 100 : null;
};
const normalizeShippingOptions = (value = []) => (Array.isArray(value) ? value : []).map((item) => ({
    type: text(item?.type).toLowerCase() === 'courier' ? 'courier' : 'delivery',
    label: text(item?.label || item?.name || (text(item?.type).toLowerCase() === 'courier' ? 'Courier' : 'Delivery propio')),
    cost: normalizeMoneyInput(item?.cost) ?? 0,
    free_from: normalizeMoneyInput(item?.free_from ?? item?.freeFrom),
    estimated_time: text(item?.estimated_time || item?.estimatedTime),
...
const paymentMethodLabels = { yape: 'Yape', plin: 'Plin', bank_transfer: 'Transferencia bancaria', credit_card: 'Tarjeta de crédito' };

const normalizePaymentModality = (value = {}) => ({
...
function normZone(item = {}) {
    ...
    return { ruleId: upper(item.ruleId || item.rule_id || ''), name: text(item.name), color: text(item.color || '#00A884') || '#00A884', rulesJson: rules && typeof rules === 'object' && !Array.isArray(rules) ? rules : {}, shippingOptions: normalizeShippingOptions(item.shippingOptions || item.shipping_options), paymentMethods: normalizePaymentMethods(item.paymentMethods || item.payment_methods), paymentModality: normalizePaymentModality(item.paymentModality || item.payment_modality), isActive: item.isActive !== false && item.is_active !== false };
}
...
function zoneForm(item = null) {
    ...
    const r = z.rulesJson || {};
    return { ...EMPTY_ZONE, ruleId: z.ruleId, name: z.name, color: z.color, departments: uniq(r.departments || r.departmentNames || r.departamentos || []), provinces: uniq(r.provinces || r.provinceNames || r.provincias || []), districts: uniq(r.districts || r.districtNames || r.distritos || []), shippingOptions: normalizeShippingOptions(z.shippingOptions), paymentMethods: normalizePaymentMethods(z.paymentMethods), paymentModality: normalizePaymentModality(z.paymentModality), isActive: z.isActive !== false };
}
function zoneFormToRule(form = {}, fallbackRuleId = '') {
    return normZone({
        ...
        shippingOptions: normalizeShippingOptions(form.shippingOptions),
        paymentMethods: normalizePaymentMethods(form.paymentMethods),
        paymentModality: normalizePaymentModality(form.paymentModality)
    });
}
...
function runLabelsAction(runSectionAction, runAction, actionKey, label, action) {
...
function GlobalPanel({ busy, requestJson, runAction, runSectionAction, setError, isSuperAdmin }) {
...
function ZonePanel({ busy, requestJson, runAction, runSectionAction, setError, canManageZones, canViewZones = true, tenantScopeLocked, settingsTenantId, ensureSectionData, isLoading, getError, getReloadToken, forceReload }) {
    ...
    useEffect(() => {
        if (typeof ensureSectionData !== 'function') {
            load().catch((e) => setError?.(String(e?.message || e || 'No se pudieron cargar zonas.')));
            return;
        }
    ...
    const addShippingOption = () => setForm((p) => ({ ...p, shippingOptions: [...normalizeShippingOptions(p.shippingOptions), { type: 'delivery', label: 'Delivery propio', cost: 0, free_from: null, estimated_time: '', is_active: true }] }));
    const updateShippingOption = (index, patch) => setForm((p) => ({ ...p, shippingOptions: normalizeShippingOptions(p.shippingOptions).map((item, itemIndex) => (itemIndex === index ? { ...item, ...patch } : item)) }));
    const removeShippingOption = (index) => setForm((p) => ({ ...p, shippingOptions: normalizeShippingOptions(p.shippingOptions).filter((_, itemIndex) => itemIndex !== index) }));
    const togglePaymentMethod = (method) => setForm((p) => ({ ...p, paymentMethods: { ...normalizePaymentMethods(p.paymentMethods), [method]: !normalizePaymentMethods(p.paymentMethods)[method] } }));
    const togglePaymentModality = (modality) => setForm((p) => ({ ...p, paymentModality: { ...normalizePaymentModality(p.paymentModality), [modality]: !normalizePaymentModality(p.paymentModality)[modality] } }));
    const shippingOptions = normalizeShippingOptions(form.shippingOptions);
    const paymentMethods = normalizePaymentMethods(form.paymentMethods);
    const paymentModality = normalizePaymentModality(form.paymentModality);
    ...
    const right = <SaasDetailPanel title={detail ? selected.name || 'Zona' : mode === 'create' ? 'Nueva zona' : 'Editar zona'} subtitle={detail ? 'Regla geografica del tenant.' : 'Agrega departamentos, provincias y distritos con selectores jerarquicos.'} className="saas-labels-detail-panel" actions={<>{detail && canManageZones ? <button type="button" disabled={busy} onClick={openEdit}>Editar</button> : null}<button type="button" className="saas-btn-cancel" disabled={busy} onClick={close}>Volver</button></>}>
        {detail ? <>
            ...
            <SaasDetailPanelSection title="Opciones de envio"><div className="saas-admin-related-list">{shippingOptions.length ? shippingOptions.map((option, index) => <div key={`shipping_read_${index}`} className="saas-admin-detail-field"><span>{option.type === 'courier' ? `Courier ${option.label}` : option.label}</span><strong>{option.is_active === false ? 'Inactivo' : `S/ ${Number(option.cost || 0).toFixed(2)}${option.free_from ? ` · gratis desde S/ ${Number(option.free_from).toFixed(2)}` : ''}${option.estimated_time ? ` · ${option.estimated_time}` : ''}`}</strong></div>) : <small>Sin opciones de envio configuradas.</small>}</div></SaasDetailPanelSection>
            <SaasDetailPanelSection title="Metodos de pago"><div className="saas-admin-detail-grid">{paymentMethodEntries().map(([method, label]) => <div key={`payment_read_${method}`} className="saas-admin-detail-field"><span>{label}</span><strong>{paymentMethods[method] ? 'Activo' : 'Inactivo'}</strong></div>)}</div></SaasDetailPanelSection>
            <SaasDetailPanelSection title="Modalidad de pago"><div className="saas-admin-detail-grid">{Object.entries(paymentModalityLabels).map(([method, label]) => <div key={`payment_modality_read_${method}`} className="saas-admin-detail-field"><span>{label}</span><strong>{paymentModality[method] ? 'Activo' : 'Inactivo'}</strong></div>)}</div></SaasDetailPanelSection>
        </> : <>
            ...
            <SaasDetailPanelSection title="Opciones de envio"><div className="saas-admin-related-list">{shippingOptions.map((option, index) => <div key={`shipping_edit_${index}`} className="saas-admin-related-block"><div className="saas-admin-form-row"><select value={option.type} disabled={busy} onChange={(e) => updateShippingOption(index, { type: e.target.value, label: e.target.value === 'courier' && option.label === 'Delivery propio' ? 'Shalom' : option.label })}><option value="delivery">Delivery propio</option><option value="courier">Courier</option></select><input value={option.label} onChange={(e) => updateShippingOption(index, { label: e.target.value })} placeholder={option.type === 'courier' ? 'Nombre del courier, ej: Shalom' : 'Nombre / label'} disabled={busy} /></div><div className="saas-admin-form-row"><input type="number" min="0" step="0.1" value={moneyValue(option.cost)} onChange={(e) => updateShippingOption(index, { cost: e.target.value })} placeholder="Costo S/" disabled={busy} /><input type="number" min="0" step="0.1" value={moneyValue(option.free_from)} onChange={(e) => updateShippingOption(index, { free_from: e.target.value })} placeholder="Gratis desde S/ (opcional)" disabled={busy} /></div><div className="saas-admin-form-row"><input value={option.estimated_time} onChange={(e) => updateShippingOption(index, { estimated_time: e.target.value })} placeholder="Tiempo estimado, ej: 1-2 dias habiles" disabled={busy} /><label className="saas-admin-module-toggle"><input type="checkbox" checked={option.is_active !== false} onChange={(e) => updateShippingOption(index, { is_active: e.target.checked })} disabled={busy} /><span>Activo</span></label><button type="button" className="saas-btn-cancel" disabled={busy} onClick={() => removeShippingOption(index)}>Eliminar</button></div></div>)}{shippingOptions.length === 0 ? <small>Sin opciones. Agrega delivery propio o courier segun la zona.</small> : null}<div className="saas-admin-form-row"><button type="button" className="saas-btn-cancel" disabled={busy} onClick={addShippingOption}>+ Agregar opcion de envio</button></div></div></SaasDetailPanelSection>
            <SaasDetailPanelSection title="Metodos de pago"><div className="saas-admin-modules">{paymentMethodEntries().map(([method, label]) => <label key={`payment_edit_${method}`} className="saas-admin-module-toggle"><input type="checkbox" checked={paymentMethods[method] === true} onChange={() => togglePaymentMethod(method)} disabled={busy} /><span>{label}{method === 'credit_card' ? ' · Se acepta si el cliente lo solicita' : ''}</span></label>)}</div></SaasDetailPanelSection>
            <SaasDetailPanelSection title="Modalidad de pago"><div className="saas-admin-modules">{Object.entries(paymentModalityLabels).map(([method, label]) => <label key={`payment_modality_edit_${method}`} className="saas-admin-module-toggle"><input type="checkbox" checked={paymentModality[method] === true} onChange={() => togglePaymentModality(method)} disabled={busy} /><span>{label}</span></label>)}</div></SaasDetailPanelSection>
            <SaasDetailPanelSection title="Acciones"><div className="saas-admin-form-row saas-admin-form-row--actions"><button type="button" disabled={busy || !canManageZones || !text(form.name)} onClick={() => runLabelsAction(runSectionAction, runAction, 'save_zone', 'Zona guardada', async () => { const payload = { ruleId: form.ruleId || undefined, name: form.name, color: form.color, isActive: form.isActive !== false, rulesJson: { districts: uniq(form.districts), districtNames: uniq(form.districts), provinces: uniq(form.provinces), provinceNames: uniq(form.provinces), departments: uniq(form.departments), departmentNames: uniq(form.departments) }, shippingOptions: normalizeShippingOptions(form.shippingOptions), paymentMethods: normalizePaymentMethods(form.paymentMethods), paymentModality: normalizePaymentModality(form.paymentModality) }; const saved = await saveTenantZoneRule(requestJson, payload); const savedRule = normZone(saved?.item || saved?.rule || saved?.zoneRule || { ...payload, ruleId: form.ruleId }); const next = upper(savedRule.ruleId || form.ruleId || ''); if (next) { const optimisticRule = zoneFormToRule(form, next); const mergedRule = normZone({ ...optimisticRule, ...savedRule, ruleId: next }); setCachedItems(upsertById(items, mergedRule, 'ruleId')); setSelectedId(next); setForm(zoneForm(mergedRule)); setMode('detail'); } else close(); })}>Guardar zona</button>{form.ruleId ? <button type="button" className="danger" disabled={busy || !canManageZones} onClick={() => runLabelsAction(runSectionAction, runAction, 'delete_zone', 'Zona eliminada', async () => { await deleteTenantZoneRule(requestJson, form.ruleId); const nextItems = items.filter((item) => upper(item.ruleId) !== upper(form.ruleId)); setCachedItems(nextItems); close(); })}>Eliminar</button> : null}<button type="button" className="saas-btn-cancel" disabled={busy} onClick={close}>Cancelar</button></div></SaasDetailPanelSection>
        </>}
    </SaasDetailPanel>;
    return <SaasEntityPage title="Zonas" sectionKey="tenant_zone_rules_inner" rows={buildLabelRows(visible, 'ruleId', 'zone')} columns={LABEL_TABLE_COLUMNS} selectedId={mode === 'list' ? '' : selectedId} onSelect={(row) => openDetail(row)} onClose={close} renderDetail={() => right} renderForm={() => right} mode={mode === 'create' || mode === 'edit' ? 'form' : 'detail'} dirty={mode === 'create' || mode === 'edit'} requestJson={requestJson} loading={sectionLoading} emptyText={sectionError || 'No hay zonas para mostrar.'} searchPlaceholder="Buscar zona" actions={[...(canManageZones ? [{ key: 'recalculate', label: recalc ? `Recalc: ${recalc.assigned || 0}` : 'Recalcular zonas', onClick: () => runLabelsAction(runSectionAction, runAction, 'recalculate_zones', 'Zonas recalculadas', async () => setRecalc(await recalculateTenantZones(requestJson))), disabled: busy }, { key: 'create', label: 'Nuevo', onClick: openCreate, disabled: busy }] : []), { key: 'reload', label: sectionError ? 'Reintentar' : 'Recargar', onClick: () => (typeof forceReload === 'function' ? forceReload(lazySectionId) : load({ force: true }).catch((e) => setError?.(String(e?.message || e)))), disabled: busy || sectionLoading }]} filters={ZONE_FILTERS} layoutClassName="saas-labels-layout" detailShell={false} hideCloseButton />;
}

function OperationalPanel({ context }) {
    const { busy, loadingLabels, settingsTenantId, loadTenantLabels, setError, canManageLabels, openTenantLabelCreate, labelSearch, setLabelSearch, visibleTenantLabels, selectedTenantLabel, labelPanelMode, setSelectedLabelId, setLabelPanelMode, openTenantLabelEdit, runAction, runSectionAction, deactivateTenantLabel, waModules, labelForm, setLabelForm, normalizeTenantLabelColor, DEFAULT_LABEL_COLORS, toggleModuleInLabelForm, saveTenantLabel, cancelTenantLabelEdit } = context;
...
function OperationalPanelUnified({ context }) {
    ...
    useEffect(() => {
        ...
        if (typeof ensureSectionData !== 'function') {
            loadTenantLabels?.(settingsTenantId).catch((e) => setError?.(String(e?.message || e || 'No se pudieron cargar etiquetas.')));
            return;
        }
        void ensureSectionData(
            lazySectionId,
            () => loadTenantLabels?.(settingsTenantId),
            {
    ...
    }, [canViewLabels, ensureSectionData, loadTenantLabels, sectionReloadToken, setError, settingsTenantId]);
...
export default function TenantLabelsSection(props = {}) {
    ...
    if (isGlobalScope) {
        return <GlobalPanel {...context} isSuperAdmin={isSuperAdmin} />;
    }
    if (noTenantAccess) {
        return <Empty title="Sin permisos" body="No tienes acceso a etiquetas operativas de este tenant." />;
    }
    return <>{!tenantAvailable ? <div className="saas-labels-tenant-hint">Selecciona una empresa para administrar etiquetas operativas.</div> : null}<OperationalPanelUnified context={context} /></>;
}
