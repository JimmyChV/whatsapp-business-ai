...
import { SaasDataTable, SaasDetailPanel, SaasDetailPanelSection, SaasTableDetailLayout, SaasViewHeader } from '../components/layout';
...
function LabelsTable({ items = [], selectedId = '', idField = 'id', kind = 'label', emptyText = 'No hay etiquetas para mostrar.', onSelect }) {
    ...
    const columns = [
        ...
        { key: 'metaText', label: kind === 'global' ? 'Estado comercial' : 'Alcance', minWidth: '170px' },
...
function GlobalPanel({ busy, requestJson, runAction, setError, isSuperAdmin }) {
    ...
    const left = <div className="saas-labels-pane"><LabelsTable kind="global" items={visible} selectedId={selectedId} idField="id" emptyText="No hay etiquetas globales para mostrar." onSelect={openDetail} /></div>;
    const right = <SaasDetailPanel title={mode === 'create' ? 'Nueva global' : mode === 'edit' ? 'Editar global' : selected?.name || 'Etiqueta global'} subtitle={mode === 'detail' ? 'Etiqueta comercial global.' : 'Define nombre, color, orden y estado comercial asociado.'} className="saas-labels-detail-panel" actions={<>{mode === 'detail' ? <button type="button" disabled={busy} onClick={openEdit}>Editar</button> : null}<button type="button" className="saas-btn-cancel" disabled={busy} onClick={close}>Cerrar</button></>}>
        {mode === 'detail' && selected ? <SaasDetailPanelSection title="Detalle"><div className="saas-admin-detail-grid"><div className="saas-admin-detail-field"><span>Codigo</span><strong>{selected.id}</strong></div><div className="saas-admin-detail-field"><span>Nombre</span><strong>{selected.name || '-'}</strong></div><div className="saas-admin-detail-field"><span>Estado comercial</span><strong>{selected.commercialStatusKey || '-'}</strong></div><div className="saas-admin-detail-field"><span>Orden</span><strong>{selected.sortOrder}</strong></div><div className="saas-admin-detail-field"><span>Estado</span><strong>{selected.isActive ? 'Activa' : 'Inactiva'}</strong></div><div className="saas-admin-detail-field"><span>Color</span><strong><Dot color={selected.color} />{selected.color}</strong></div></div>{selected.description ? <p className="saas-labels-detail-description">{selected.description}</p> : null}</SaasDetailPanelSection> : <SaasDetailPanelSection title="Formulario"><div className="saas-admin-form-row"><input value={form.name} onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} placeholder="Nombre" disabled={busy} /><input value={form.commercialStatusKey} onChange={(e) => setForm((p) => ({ ...p, commercialStatusKey: e.target.value }))} placeholder="Estado comercial: nuevo, cotizado..." disabled={busy} /></div><div className="saas-admin-form-row"><ColorPicker value={form.color} disabled={busy} onChange={(color) => setForm((p) => ({ ...p, color }))} /><input type="number" value={form.sortOrder} onChange={(e) => setForm((p) => ({ ...p, sortOrder: e.target.value }))} placeholder="Orden" disabled={busy} /></div><textarea className="saas-labels-textarea" value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} placeholder="Descripcion" disabled={busy} /><label className="saas-admin-module-toggle"><input type="checkbox" checked={form.isActive !== false} onChange={(e) => setForm((p) => ({ ...p, isActive: e.target.checked }))} disabled={busy} /><span>Activa</span></label><div className="saas-admin-form-row saas-admin-form-row--actions"><button type="button" disabled={busy || !text(form.name)} onClick={() => runAction?.('Etiqueta global guardada', async () => { const saved = await saveGlobalLabel(requestJson, { id: form.id || undefined, name: form.name, color: form.color, description: form.description, commercialStatusKey: form.commercialStatusKey, sortOrder: Number(form.sortOrder || 100) || 100, isActive: form.isActive !== false }); await load(); const next = upper(saved?.item?.id || form.id || ''); if (next) { setSelectedId(next); setMode('detail'); } else close(); })}>Guardar global</button>{form.id ? <button type="button" className="danger" disabled={busy} onClick={() => runAction?.('Etiqueta global eliminada', async () => { await deleteGlobalLabel(requestJson, form.id); await load(); close(); })}>Eliminar</button> : null}<button type="button" className="saas-btn-cancel" disabled={busy} onClick={close}>Cancelar</button></div></SaasDetailPanelSection>}
    </SaasDetailPanel>;
    return <SaasTableDetailLayout selectedId={mode === 'list' ? '' : selectedId} className="saas-labels-layout" header={<SaasViewHeader title="Globales comerciales" count={visible.length} searchValue={search} onSearchChange={setSearch} searchPlaceholder="Buscar etiqueta global" actions={[{ key: 'reload', label: 'Recargar', onClick: () => load().catch((e) => setError?.(String(e?.message || e))), disabled: busy || loading }, { key: 'create', label: 'Nueva global', onClick: openCreate, disabled: busy }]} />} left={left} right={right} />;
}
...
function ZonePanel({ busy, requestJson, runAction, setError, canManageLabels, tenantScopeLocked, settingsTenantId }) {
    ...
    useEffect(() => {
        ...
        requestJson('/api/tenant/customer-catalogs/geo', { method: 'GET' }).then((payload) => {
            if (!cancelled) setGeo({ departments: Array.isArray(payload?.departments) ? payload.departments : [], provinces: Array.isArray(payload?.provinces) ? payload.provinces : [], districts: Array.isArray(payload?.districts) ? payload.districts : [] });
        }).catch(() => { if (!cancelled) setGeo({ departments: [], provinces: [], districts: [] }); }).finally(() => { if (!cancelled) setGeoLoading(false); });
        return () => { cancelled = true; };
    }, [requestJson, settingsTenantId, tenantScopeLocked]);
    ...
    const right = <SaasDetailPanel title={detail ? selected.name || 'Zona' : mode === 'create' ? 'Nueva zona' : 'Editar zona'} subtitle={detail ? 'Regla geografica del tenant.' : 'Agrega departamentos, provincias y distritos con selectores jerarquicos.'} className="saas-labels-detail-panel" actions={<>{detail ? <button type="button" disabled={busy || !canManageLabels} onClick={openEdit}>Editar</button> : null}<button type="button" className="saas-btn-cancel" disabled={busy} onClick={close}>Cerrar</button></>}>
    ...
    return <SaasTableDetailLayout selectedId={mode === 'list' ? '' : selectedId} className="saas-labels-layout" header={<SaasViewHeader title="Zonas" count={visible.length} searchValue={search} onSearchChange={setSearch} searchPlaceholder="Buscar zona" actions={[{ key: 'recalculate', label: recalc ? `Recalc: ${recalc.assigned || 0}` : 'Recalcular zonas', onClick: () => runAction?.('Zonas recalculadas', async () => setRecalc(await recalculateTenantZones(requestJson))), disabled: busy || !canManageLabels }, { key: 'reload', label: 'Recargar', onClick: () => load({ force: true }).catch((e) => setError?.(String(e?.message || e))), disabled: busy || loading }, { key: 'create', label: 'Nueva zona', onClick: openCreate, disabled: busy || !canManageLabels }]} />} left={left} right={right} />;
}
