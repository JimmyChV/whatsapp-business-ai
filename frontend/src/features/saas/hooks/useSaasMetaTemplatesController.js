...
export default function useSaasMetaTemplatesController({
    ...
    const createTemplate = useCallback(async ({ moduleId, templatePayload, useCase = 'both', variableMapJson = {}, reload = true } = {}) => {
        if (typeof requestJson !== 'function') throw new Error('requestJson no disponible.');

        setLoadingCreate(true);
        setCreateError('');
        try {
            const response = await createMetaTemplate(requestJson, { moduleId, templatePayload, useCase, variableMapJson });
