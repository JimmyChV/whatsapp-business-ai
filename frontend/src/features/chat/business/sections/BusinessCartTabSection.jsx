        ...
        <div className="cart-tab-shell" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            {cart.length > 0 && (
                <div className="cart-tab-footer" style={{ padding: '10px 9px', borderTop: '1px solid var(--border-color)', background: tone.shellSurface, display: 'flex', flexDirection: 'column', gap: '10px', flexShrink: 0 }}>
                    <button
                        type="button"
                        onClick={() => typeof onBackToCatalog === 'function' && onBackToCatalog()}
                        style={{ width: '100%', background: tone.controlSurface, border: `1px solid ${tone.controlBorder}`, color: 'var(--text-primary)', borderRadius: '8px', padding: '8px 10px', cursor: 'pointer', fontSize: '0.8rem', fontWeight: 800 }}
                    >
                        ← Seguir agregando
                    </button>
