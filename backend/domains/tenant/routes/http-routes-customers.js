...
const logisticsAdvisorService = require('../services/logistics-advisor.service');
...
function registerTenantCustomerHttpRoutes({
    ...
    app.post('/api/tenant/coverage/detail', async (req, res) => {
        try {
            if (!ensureAuthenticated(req, res, authService)) return;
            if (!hasCoverageResolveAccess(req)) return res.status(403).json({ ok: false, error: 'No autorizado.' });
            const tenantId = String(req?.tenantContext?.id || 'default').trim() || 'default';
            ...
            return res.json({ ok: true, tenantId, ...decision });
        } catch (error) {
            return res.status(500).json({ ok: false, error: String(error?.message || 'No se pudo generar el detalle de cobertura.') });
        }
    });
